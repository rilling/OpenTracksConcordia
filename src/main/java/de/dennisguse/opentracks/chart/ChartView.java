...
public class ChartView extends View {
    ...
    private final GestureDetectorCompat detectorScrollFlingTab = new GestureDetectorCompat(getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onSingleTapConfirmed(MotionEvent event) {
            ...
            if (Math.abs(event.getY() - topBorder - spacer_str - markerHeight / 2f) < markerHeight) {
    ...
    private void drawMarker(Canvas canvas) {
        synchronized (markers) {
            for (Marker marker : markers) {
                ...
                canvas.drawLine(x, (float)(topBorder) + spacer_str + markerHeight / 2, x, (float)(topBorder) + effectiveHeight, markerPaint);
                canvas.translate(x - (markerWidth * MARKER_X_ANCHOR), (float)(topBorder) + spacer_str);
    ...
    private void drawSeriesTitles(Canvas canvas) {
        ...
        for (ChartValueSeries chartValueSeries : seriesList) {
            if (chartValueSeries.isEnabled() && chartValueSeries.hasData() || allowIfEmpty(chartValueSeries)) {
                ...
                int y = topBorder - spacer_str - (lines - count) * (lineHeight + spacer_str);
    ...
    private void drawXAxis(Canvas canvas) {
        ...
        canvas.drawText(label, x + effectiveWidth + spacer_str, y + yOffset, axisPaint);
        ...
        for (double markerPosition : getXAxisMarkerPositions(interval)) {
            drawXAxisMarker(canvas, markerPosition, numberFormat, spacer_str + yOffset);
        }
    }
    ...
    private void drawYAxis(Canvas canvas) {
        ...
        //TODO
        int markerXPosition = x - spacer_str;
        for (int i = 0; i < seriesList.size(); i++) {
            int index = seriesList.size() - 1 - i;
            ChartValueSeries chartValueSeries = seriesList.get(index);
            if (chartValueSeries.isEnabled() && chartValueSeries.hasData() || allowIfEmpty(chartValueSeries)) {
                markerXPosition -= drawYAxisMarkers(chartValueSeries, canvas, markerXPosition) + spacer_str;
            }
        }
    }
    ...
    private void updateDimensions() {
        ...
        for (ChartValueSeries chartValueSeries : seriesList) {
            if (chartValueSeries.isEnabled() && chartValueSeries.hasData() || allowIfEmpty(chartValueSeries)) {
                ...
                markerLength += rect.width() + spacer_str;
            }
        }

        leftBorder = (int) (density * BORDER + markerLength);
        int[] titleDimensions = getTitleDimensions();
        topBorder = (int) (density * BORDER + titleDimensions[0] * (titleDimensions[1] + spacer_str));
        Rect xAxisLabelRect = getRect(axisPaint, getXAxisLabel());
        // border + x axis marker + spacer_str + .5 x axis label
        bottomBorder = (int) (density * BORDER + getRect(xAxisMarkerPaint, "1").height() + spacer_str + (xAxisLabelRect.height() / 2));
        rightBorder = (int) (density * BORDER + xAxisLabelRect.width() + spacer_str);
    ...
    private void updateEffectiveDimensions() {
        ...
        effectiveHeight = Math.max(0, height - topBorder - bottomBorder - spacer_str);
    }
